...
其中 $\left\{ \begin{array}{cc}  A=a+xp & |A|<\frac p 2\\ B=b+yp & |B|<\frac p 2 \end{array}\right.$（因为 $|z|<\dfrac p 2$ 是一个完全剩余系，$z$ 叫做最小余数）
...
现在要把 $n$ 看成模数，取 $A$ 为 $a$ 的最小余数，$B$ 为 $b$ 的最小余数，也就是 $\left\{\begin{array}{cc}  A=a+xn&|A|\le \frac n 2\\ B=b+yn&|B|\le \frac n 2 \end{array}\right.$
